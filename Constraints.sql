...
-- 재약조건 : 데이터베이스 테이블 컬럼에 삽입, 수정, 삭제 시 적용되는 규칙

-- NOT NULL 제약조건 : 해당 컬럼에 null을 포함하지 못하도록 제약

CREATE TABLE not_null_table (
	null_colmun INT NULL,
...
-- UNIQUE 제약조건 : 해당 컬럼에 중복된 데이터를 포함할 수 없도록 하는 
-- (자기 자신 테이블 INSERT, UPDATE) 


CREATE TABLE unique_table (
	unisque_column INT UNIQUE,
...
-- 제약조건은 NOT NULL, UNIQUE 제약조건을 모두 가지고 있음
...
-- FOREIGNKEY 제약조건으로 참조되어지고 있는 테이블의 컬럼 변경 작업이 불가능
...
-- AUTO 기본키가 정수형일 때 기본키의 값을 1씩 증가하는 값으로 자동 지정
...
SELECT * FROM optional _foreign_table;
