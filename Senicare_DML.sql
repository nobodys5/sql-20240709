...
UPDATE tel_auth SET auth_number = '0123' WHERE tel_number = '0101';
...
SELECT * FROM tel_auth WHERE tel_auth = '01012345678' AND auth = '0123';
...
INSERT INTO tel_auth VALUES (tel_number, phone_number);

-- 인증번호 확인 
-- 최종사용자 (인증번호)
-- 전화번호 인증 테이블에서 전화번호에 해당하는 인증번호가 사용자가 입력한 인증번호와 같은지 비교
-- 필요한 추가 데이터 : 전화번호 (화면에서 가져옴)
SELECT * FROM tel_auth WHERE tel_number = '1' AND auth_number = '2';
